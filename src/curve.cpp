...
void BezierCurve::addPoint(Vec2 position) { addPoint(BezierPoint(position)); }
...
void BezierCurve::forceUpdateSegments() {
  ...
  m_segments.clear();
  for (std::vector<BezierPoint>::iterator it = m_bez_points.begin(),
                                          sec2last = --m_bez_points.end();
       it != sec2last; ++it) {
